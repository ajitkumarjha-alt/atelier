    ...
    <div className="space-y-6">
      <CalcFieldGroup label="System Parameters">
        <CalcField label="Design Type" value={params.designType} onChange={v => onChange('designType', v)}
          options={[{value:'RISING_MAIN',label:'Rising Main'}]} />
      ...
      <CalcFieldGroup label="Conductor">
        ...
        <CalcField label="Installation" value={params.installationMethod} onChange={v => onChange('installationMethod', v)}
          options={['VERTICAL_BUS_DUCT','CABLE_SHAFT','CABLE_TRAY'].map(m=>({value:m,label:m.replace(/_/g,' ')}))} />
      </CalcFieldGroup>
      <div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <ResultCard title="Busbar Rating" value={results.selectedBusbar?.ratingA || '-'} unit="A" icon={TrendingUp} color="blue" />
      ...
      {results.selectedBusbar && (
        <div className="bg-white rounded-xl shadow-sm border border-lodha-sand p-6">
          <h3 className="text-base font-semibold text-lodha-grey mb-3">Selected Conductor</h3>
...
export default function RisingMainDesign() {
  return (
    <MepCalculatorShell
      calculationType="rising_main"
      title="Rising Main Design"
      icon={TrendingUp}
