    ...
    <div className="space-y-6">
      <CalcFieldGroup label="System Parameters">
        <CalcField label="Design Type" value={params.designType} onChange={v => onChange('designType', v)}
          options={[{value:'BUS_RISER',label:'Bus Riser'}]} />
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <ResultCard title="Busbar Rating" value={results.selectedBusbar?.ratingA || '-'} unit="A" icon={Layers} color="blue" />
        ...
        <ResultCard title="Sections" value={results.numberOfSections || 0} icon={TrendingUp} color="orange" />
      </div>
      {results.selectedBusbar && (
        <div className="bg-white rounded-xl shadow-sm border border-lodha-sand p-6">
          <h3 className="text-base font-semibold text-lodha-grey mb-3">Selected Bus Riser</h3>
      ...
      {floors.length > 0 && (
        <ResultTable
          title="Floor-wise Bus Riser Analysis"
